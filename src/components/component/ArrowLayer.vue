...
watch(input, (newVal) => {
  ...
  emit('percent', min / stratagem.value.length)
...
<template>
  <div class="m-0 flex h-full w-full flex-row items-center justify-center gap-0 p-0">
    <div
      v-for="(obj, index) in stratagem"
      :key="index"
      class="h-[10vh] w-[4.3vw]"
    >
      <svg
        v-if="obj === Direction.UP"
      ...
      >
        <path
          :style="fillArrowColor(index)"
          :class="{ 'fill-game-red': errorIndex.includes(index) }"
          fill="#ffffff"
